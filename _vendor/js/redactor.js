  ...
  Redactor.prototype = {
    ...
    keydown: function()
    {
      this.$editor.keydown($.proxy(function(e)
      {
        ...
        var isNotEmpty = ($(node).has('br').length) ? false : true;
        var isLastChild = $(node).is(':last-child');
        var isNotSection = ($(node).parent('.redactor_editor').length) ? true : false;
        var isNotComment = ($(node).parent('.media__content').length) ? false : true;
        console.log(isNotEmpty + ', ' + isLastChild + ', ' + isNotSection + ', ' + isNotComment);
        if (isLastChild && isNotEmpty && isNotSection && isNotComment ){
          $("body").animate({ scrollTop: $(document).height() }, 50).dequeue();
        }
    ...
    syncCode: function()
    {
      this.$editor.find('pre').addClass('prettyprint');
      prettyPrint();
    ...
    {
      var button = $('<a href="javascript:void(null);" name="' + s.title + '" class="redactor_btn_' + key + '"></a>');
        ...
        if (this.opts.imageGetJson !== false)
        {
          $.getJSON(this.opts.imageGetJson, $.proxy(function(data) {
            ...
            $.each(data, $.proxy(function(key, val)
            {
              ...
              var img = $('<img src="' + val.thumb + '" class="redactorfolder redactorfolder' + folderkey + '" rel="' + val.image + '" name="' + thumbtitle + '" />');
    ...
    {
      this._imageSet('<img src="' + $(e.target).attr('rel') + '" alt="' + $(e.target).attr('name') + '" />', true);
    },
